...
def part_2():
    ...
    current_nodes = find_start_nodes(nodes)
    print("Start nodes", len(current_nodes))

    steps = 0

    while not is_end_nodes(current_nodes):
        if steps % 1000 == 0:
            print("Step", steps, end="\r")
        sprint(
            "Step ",
            steps,
            "Current nodes",
        ...
        for i, current in enumerate(current_nodes):
            node_set = nodes[current]
            current_nodes[i] = node_set[0] if instruction == "L" else node_set[1]
        steps += 1
    return steps
